...
- **This machine routinely has 20–40 of these alive per account at once**
  (mostly idle resumed shells, some going back over a week) — a full list
  would be clutter, not a "what's active" view. Only the **6
  most-recently-active** (by the session's own `updatedAt`) are kept per
  account.
...
- `bar/ClaudeUsagePill.qml` — compact per-account **session%** only (fixed
  ...
  RGB but keeps the source alpha shape) to `Theme.cyan` — the scheme's
  primary accent, generated by `gen-theme.py` from the current wallpaper
  (see theming.md) and the same color `Clock.qml` uses — rather than
  staying pinned to Claude's brand orange. `Theme.text` was tried first
  and rejected: at icon size it just read as flat grey against this bar's
  dark background.
