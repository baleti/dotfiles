#!/usr/bin/env bash
# Steps to the next/previous image in ~/pictures (sorted, wrapping), based
# on wherever set-wallpaper.sh last recorded as the source. No current
# record (nothing ever set, or the file's no longer in ~/pictures) just
...
mapfile -t candidates < <(find "$dir" -maxdepth 1 -type f \
