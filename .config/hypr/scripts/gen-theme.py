...
2026-08-28 for the GTK root-cause writeup):
  ...
  - ~/.config/alacritty/colors.toml -- just the [colors.primary/normal/bright]
    tables, imported by alacritty.toml via `[general] import` (alacritty.yml
    is untouched dead weight -- alacritty only reads the .toml).
...
ALACRITTY_COLORS = Path.home() / ".config/alacritty/colors.toml"
...
HYPR_BORDER_COLORS = STATE_DIR / "hyprland-border-colors.lua"
...
def theme_hyprland_borders(out: dict) -> None:
    """Applied live via `hyprctl eval` for an instant update, AND written to
    HYPR_BORDER_COLORS -- a small generated Lua file, NOT appearance.lua
    itself (editing that live Lua config unattended was judged too risky).
    appearance.lua loadfile()s HYPR_BORDER_COLORS at parse time and falls
    back to its own hardcoded defaults if it's missing, the same pattern
    hyprland.lua already uses to re-merge nsxiv's generated X-resources on
    every load. Without this file, a plain `hyprctl reload` re-parses
    appearance.lua's hardcoded rgba(33ccff...) default and silently wipes
    out whatever the live `hyprctl eval` call had applied -- confirmed
    2026-08-31 as the cause of the focused-window border "reverting to the
    wrong color" after every reload once wallpaper-watch.sh (which used to
    paper over this by reapplying the live eval on its own poll loop) was
    retired 2026-08-30.
    ...
    atomic_write(
        ...
        "-- Read by appearance.lua at parse time (every `hyprctl reload` included)\n"
        "-- so reload doesn't revert to appearance.lua's hardcoded defaults.\n"
        "return {\n"
        f'    active_border   = "{active_border}",\n'
        f'    inactive_border = "{inactive_border}",\n'
        "}\n",
...
def theme_alacritty(out: dict) -> None:
    """Writes a standalone ~/.config/alacritty/colors.toml holding just the
    [colors.primary/normal/bright] tables; alacritty.toml pulls it in via
    `[general] import`. Split out (rather than patched in place) so the
    wallpaper-driven recolour never touches the hand-maintained, git-tracked
    alacritty.toml -- `live_config_reload` re-reads imported files too, so
    the live update is unchanged. alacritty.yml alongside is dead weight:
    alacritty only reads the .toml."""
    series = out["seriesPalette"]
    toml = f"""# Generated by ~/.config/hypr/scripts/gen-theme.py -- do not edit.
# Imported by alacritty.toml ([general] import). Kept in its own file so the
# wallpaper-driven recolour doesn't churn the tracked alacritty.toml.

[colors.primary]
background = "{out['background']}"
foreground = "{out['onBackground']}"
...
magenta = "{series[4]}"
cyan = "{out['primary']}"
white = "{out['onSurfaceVariant']}"

[colors.bright]
black = "{out['onSurfaceVariant']}"
red = "{series[5]}"
green = "{series[0]}"
yellow = "{series[6]}"
blue = "{series[2]}"
magenta = "{series[4]}"
cyan = "{out['primary']}"
white = "{out['onBackground']}"
"""
    atomic_write(ALACRITTY_COLORS, toml)
    print(f"wrote {ALACRITTY_COLORS}")
