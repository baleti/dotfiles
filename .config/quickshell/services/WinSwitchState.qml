...
QtObject {
    ...
    function _onTab(direction) {
        ...
        root._log(`tab ${direction} (session ${root.sessionId}, locked=${root.locked})`);
        if (root.locked)
    ...
    function _onAltUp() {
        if (!root.active)
            return;
        root._log(`altup (shown=${root.shown}, locked=${root.locked}, selected=${root.selected})`);
        if (!root.locked)
