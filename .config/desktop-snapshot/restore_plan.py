...
CLAUDE_DIRS_BY_KEY = {d.name.lstrip("."): d for d in CLAUDE_DIRS}
...
# Recognizes tmux session names generated by the two headless-spawn paths
# on this machine - claude-agents-daemon.py's spawn_session() (SPAWN_SESSION_PREFIX
# "rss-agent", <prefix>-YYYYmmdd-HHMMSS-<hex>) and peer-agent's config.json tmux
# actions (session_prefix "remote-claude"/"remote-claude2"/"remote-claude3", same
# YYYYmmdd-HHMMSS-<hex> suffix), plus an older/legacy naming style seen in
# ~/.config/claude-agents/daemon.log ("claude-remote[-N]-<unix-epoch>") that
# predates the current code - not reproducible from today's source, kept here
# only so historical sessions with that name still resolve.
_AUTOMATION_SESSION_NAME_RE = re.compile(
    r"^(?:rss-agent|remote-claude[23]?)-(\d{8})-(\d{6})-[0-9a-f]+$"
)
_AUTOMATION_SESSION_EPOCH_RE = re.compile(r"^claude-remote(?:-\d+)?-(\d{10})$")


def parse_automation_session_creation_time(session_name):
    """tmux session -> the unix timestamp encoded in its own name, or None
    if the name doesn't match a recognized headless-spawn pattern. See
    resolve_resume_uuid_by_creation_time's docstring for what this feeds."""
    m = _AUTOMATION_SESSION_NAME_RE.match(session_name)
    if m:
        try:
            ...
            return None
        return dt.timestamp()
    m = _AUTOMATION_SESSION_EPOCH_RE.match(session_name)
    if m:
...
def account_uuid_map():
    """ownerAccountUuid (as stamped into a transcript's own "bridge-session"
    line) -> dir_key ("claude"/"claude2"/"claude3"), read fresh from each
    account's own <dir>/.claude.json "oauthAccount.accountUuid" field. Mirrors
    claude-agents-daemon.py's build_account_map() - kept as a small local
    copy rather than importing that module, same rationale as
    account_for_config_dir above."""
    out = {}
    for d in CLAUDE_DIRS:
        try:
            cfg = json.loads((d / ".claude.json").read_text())
        except (OSError, ValueError):
            continue
        uuid = (cfg.get("oauthAccount") or {}).get("accountUuid")
        if uuid:
            out[uuid] = d.name.lstrip(".") or "claude"
    return out


def resolve_resume_uuid_by_creation_time(created_ts, cwd, max_window=300):
    """Last-resort uuid resolution for a pane snapshot.py never captured
    any claude info for at all - not "session_id known but unresumable"
    (transcript_exists' job) and not "scrollback has an OSC-8 footer"
    (resolve_resume_uuid's job), but a pane that was sitting idle at
    capture time (current_command == "zsh") because its claude process had
    already exited or hadn't started yet - snapshot.py's per-pane claude
    capture is gated entirely on current_command == "claude" at that exact
    instant (see get_tmux_state), so a momentarily-idle automation pane
    leaves zero trace anywhere in the snapshot to resume from.

    Confirmed a real, recurring gap 2026-09-18: every claude-agents-daemon
    spawn_session()/peer-agent tmux action creates its tmux session first
    and starts claude a beat later (or the reverse, for a session that's
    between conversation turns at capture time) - both daemons always know
    the session's exact creation instant because they choose the tmux
    session name themselves (see parse_automation_session_creation_time).
    That timestamp is close to, but not identical to, the transcript's own
    first logged message: spawn_session's own initial send is ~2s after
    creation; a peer-agent session (bare `claude`, no forced first
    message) can sit for minutes before the phone's user types anything.
    Searching a wide-ish window and taking the single nearest transcript
    forward in time is a best-effort match, not a certainty - confirmed
    wrong once already (a peer-agent session ~16min before its nearest
    transcript turned out, checked via the transcript's own
    ownerAccountUuid, to belong to a different account than that tmux
    session's own config - the true transcript for that pane simply isn't
    on disk under any window tried). Callers should treat the returned
    dir_key as a strong hint, not ground truth, and this function makes no
    attempt to detect that kind of mismatch itself - see the module's
    disaster-recovery doc for how that one incident was actually resolved
    (manually, cross-checking ownerAccountUuid against each account's own
    accountUuid).

    Returns (uuid, dir_key) - dir_key is None when no bridge-session owner
    marker was found in the matched transcript (older sessions predate
    that field); the caller should fall back to its own default account in
    that case, not treat it as a resolution failure."""
    if created_ts is None:
        return None, None
    proj_name = cwd.replace("/", "-") if cwd else "-home-user1"
    seen_dirs = set()
    best = None  # (delta, path)
    for claude_dir in CLAUDE_DIRS:
        proj = claude_dir / "projects" / proj_name
        real = proj.resolve()
        if real in seen_dirs or not proj.is_dir():
            continue
        seen_dirs.add(real)
        for jf in proj.glob("*.jsonl"):
            first_ts = None
            ...
            if first_ts is None:
                continue
            delta = first_ts - created_ts
            if 0 <= delta <= max_window and (best is None or delta < best[0]):
                best = (delta, jf)
    if best is None:
        return None, None
    jf = best[1]
        ...
        with jf.open(errors="replace") as fh:
            for i, line in enumerate(fh):
                ...
                if d.get("type") == "bridge-session" and d.get("ownerAccountUuid"):
                    owner_uuid = d["ownerAccountUuid"]
                    break
    except OSError:
        pass
    dir_key = account_uuid_map().get(owner_uuid) if owner_uuid else None
    return jf.stem, dir_key
...
def _apply_tmux_body(snap, resume=False, pane_contents=None, exclude_session=None,
    ...
    if resume:
        ...
        # Second pass: sessions claude_idx above never saw at all, because
        # snapshot.py only records claude info for a pane whose
        # current_command was literally "claude" at that exact capture
        # instant (see get_tmux_state) - a headless automation session
        # that's momentarily idle (between conversation turns, or just
        # past its ~2s startup window) leaves nothing in the snapshot for
        # the first pass to find, Hyprland-windowed or not. Recognized by
        # tmux session name alone (see parse_automation_session_creation_time)
        # since these are always spawned with a name the owning daemon
        # picked itself; resolve_resume_uuid_by_creation_time then does the
        # best-effort transcript match. Confirmed a real, recurring gap
        # 2026-09-18: 9 of 11 idle claude-remote-*/rss-agent-* panes in one
        # restore recovered this way, 2 stayed genuinely unresolvable (see
        # that function's docstring).
        already_tried = {sess for (sess, _widx) in claude_idx}
        for sess in sorted(live - already_tried):
            created_ts = parse_automation_session_creation_time(sess)
            if created_ts is None:
                continue
            pane_list = subprocess.run(
                ["tmux", "list-panes", "-t", sess, "-F",
                 "#{window_index}.#{pane_index} #{pane_current_command}"],
                capture_output=True, text=True, timeout=RESUME_SUBPROCESS_TIMEOUT).stdout
            for pline in pane_list.splitlines():
                target, _, cmd = pline.partition(" ")
                if cmd == "claude":
                    continue
                window_index, _, pane_index = target.partition(".")
                uuid, dir_key = resolve_resume_uuid_by_creation_time(created_ts, cwd=None)
                if not uuid:
                    print(f"  {sess}:{target}: idle at capture, no automation-session-name "
                          f"match within the resolution window - leaving as-is", file=sys.stderr)
                    continue
                config_dir = str(CLAUDE_DIRS_BY_KEY.get(dir_key)) if dir_key else None
                inject_resume(sess, window_index, pane_index, config_dir, dir_key,
                               uuid, None, get_pane_dir, exclude_jsonl)
                time.sleep(1.5)
